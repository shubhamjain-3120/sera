...
import type { Artifact, Draft, FieldType, TemplateField } from "./types";
...
function FieldProperties({ field, update, remove }: { field: TemplateField; update: (field: TemplateField) => void; remove: (id: string) => void }) {
  ...
  return <><div className="panel-title"><div><p className="eyebrow">Field definition</p><h2>Edit field</h2></div><button className="icon-button danger" aria-label="Delete field" onClick={() => remove(field.id)}><X /></button></div><div className="form-stack"><label>Label<input value={field.label} onChange={(e) => set("label", e.target.value)} /></label><label>Type<select value={field.field_type} onChange={(e) => set("field_type", e.target.value as FieldType)}>{["text", "number", "date", "boolean", "choice", "signature", "action", "unknown"].map((type) => <option key={type}>{type}</option>)}</select></label><label>Semantic type<input placeholder="e.g. applicant.legal_name" value={field.semantic_type ?? ""} onChange={(e) => set("semantic_type", e.target.value || null)} /></label><label>Requiredness<select value={field.required === null || field.required === undefined ? "unknown" : String(field.required)} onChange={(e) => set("required", e.target.value === "unknown" ? null : e.target.value === "true")}><option value="unknown">Unknown</option><option value="true">Required</option><option value="false">Optional</option></select></label><label className="toggle-row"><span><strong>Writable</strong><small>Approved fill destination</small></span><input type="checkbox" checked={field.writable} disabled={field.field_type === "signature" || field.field_type === "action"} onChange={(e) => set("writable", e.target.checked)} /></label><div className="location-card"><span>Source location</span><strong>{field.location.kind === "pdf_rect" ? `Page ${field.location.page} · [${field.location.rect?.join(", ")}]` : `${field.location.sheet}!${field.location.cell_range}`}</strong><small>Original coordinate system retained</small></div><label>Notes<textarea rows={3} value={field.notes ?? ""} onChange={(e) => set("notes", e.target.value)} /></label></div></>;
}
...
function PdfPreview({ artifactId, fields, selected, onSelect, pages }: { artifactId: string; fields: TemplateField[]; selected: string; onSelect: (id: string) => void; pages: number }) {
  ...
  const pageFields = fields.filter((f) => f.location.kind === "pdf_rect" && f.location.page === page);
  return <div className="pdf-wrap"><div className="page-nav"><button disabled={page <= 1} onClick={() => setPage(page - 1)}>←</button><span>Page {page} of {pages}</span><button disabled={page >= pages} onClick={() => setPage(page + 1)}>→</button></div><div className="paper"><img alt={`PDF page ${page}`} src={`${API}/api/v1/artifacts/${artifactId}/pages/${page}.png?scale=1.5`} />{pageFields.map((field) => <button title={field.label} aria-label={field.label} key={field.id} onClick={() => onSelect(field.id)} className={`pdf-field ${selected === field.id ? "active" : ""}`} style={pdfRectStyle(field)} />)}</div></div>;
}

function pdfRectStyle(field: TemplateField) {
  const [x1, y1, x2, y2] = field.location.rect ?? [0, 0, 0, 0];
  const width = field.location.page_width ?? 612;
  const height = field.location.page_height ?? 792;
  const rotation = ((field.location.rotation ?? 0) % 360 + 360) % 360;
