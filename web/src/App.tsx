...
import type { Agency, Artifact, Draft, EvidenceFact, EvidenceLocation, EvidenceSource, FieldType, FormFill, FormFieldGeometry, Location, TemplateField } from "./types";
...
function FormFillView({ fill }: { fill: FormFill }) {
  ...
  const update = useMutation({ mutationFn: (input: { write_value: unknown; evidence_fact_ids?: string[]; geometry?: FormFieldGeometry }) => api.updateFormFillField(fill.id, selectedId, input), onSuccess: () => client.invalidateQueries({ queryKey: ["form-fill", fill.id] }) });
...
function AnswerBadge({ kind }: { kind: string }) {
  return <span className={`answer-badge ${kind}`}>{kind === "inferred" ? "Inferred" : kind === "tentative" ? "Review" : "Supported"}</span>;
}
