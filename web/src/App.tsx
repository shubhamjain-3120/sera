...
import type { Artifact, Draft, EvidenceFact, EvidenceLocation, EvidenceSource, FieldType, FillPlanTarget, Location, MappingCandidate, TemplateField, VerificationFinding, VerificationReport } from "./types";
...
function FillPlanLibrary() {
  ...
  return <div className="app-shell">
    <header><Brand /><WorkspaceNav /><div className="phase-badge"><span /> Phase 4 · Independent verification</div></header>
...
function FillPlanView({ plan }: { plan: Awaited<ReturnType<typeof api.fillPlan>> }) {
  ...
  return <div className="inspector mapping-inspector">
    ...
    <div className="workbench mapping-workbench">
      ...
      <aside className="properties">{report && <VerificationSummary report={report} targetId={selectedId} />}{verify.error && <div className="error-banner">{verify.error.message}</div>}{target ? <MappingProperties target={target} /> : <div className="empty-property"><Archive /><p>Select a target to inspect its proposals.</p></div>}</aside>
    </div>
  </div>;
}
